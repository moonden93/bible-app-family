...
function RoomView({ user, roomCode, onExit }) {
  ...
  async function attemptSave(arr, retries = 2) {
    try {
      await updateDoc(doc(db, 'readingRooms', roomCode), { [`reads.${user.uid}`]: arr });
    } catch (e) {
      console.error('저장 실패:', e);
      if (retries > 0) {
        setTimeout(() => attemptSave(arr, retries - 1), 500);
      } else {
